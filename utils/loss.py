...
class focal_loss(nn.Module):
    def __init__(self, alpha=0.25, gamma=2, num_classes=3, size_average=True):
        ...
        if isinstance(alpha, list):
            ...
            print("[INFO] Focal_loss alpha = {}".format(alpha))  # 将对每一类权重进行精细化赋值
            self.alpha = torch.Tensor(alpha)
        else:
            assert alpha < 1  # 如果α为一个常数,则降低第一类的影响,在目标检测中为第一类
            print("[INFO]Focal_loss alpha = {}".format(alpha))  # 将对背景类进行衰减,请在目标检测任务中使用
...
def create_loss(loss_type, **kwargs):
    if loss_type == 'CELoss':
        ...
        raise ValueError('Unsupported loss_type - `{}`, '
                         'Use CELoss, LabelSmoothCELoss, FocalLoss'.format(loss_type))
